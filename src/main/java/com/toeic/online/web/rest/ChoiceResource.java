...
/**
 * REST controller for managing {@link com.toeic.online.domain.Choice}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ChoiceResource {
    ...
    public ResponseEntity<Choice> partialUpdateChoice(
        ...
        Optional<Choice> result = choiceRepository
            ...
            .map(existingChoice -> {
                if (choice.getQuestionId() != null) {
                    existingChoice.setQuestionId(choice.getQuestionId());
                }
                if (choice.getChoiceText() != null) {
                    existingChoice.setChoiceText(choice.getChoiceText());
                }
                if (choice.getCorrected() != null) {
                    existingChoice.setCorrected(choice.getCorrected());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, choice.getId().toString())
        );
    }
